% This function processes the log files for PID 
function [] = process_pid_log(flightRunNames,baseDir,controller,properties)
    ...
    for ii = 1:length(flightRunNames)
        ...
        gains_file_path = fullfile(baseDir,flightRunNames{ii}, controller, 'params', 'gains_PID.json');
        ...
        gains.Kp_tran = GainsData.KP_translational;
        gains.Kd_tran = GainsData.KD_translational;
        gains.Ki_tran = GainsData.KI_translational;
        gains.Kp_rot = GainsData.KP_rotational;
        gains.Kd_rot = GainsData.KD_rotational;
        gains.Ki_rot = GainsData.KI_rotational;
        ...
        log.Angular_rates_x_rads = data.data(:, 12);
        log.Angular_rates_y_rads = data.data(:, 13);
        log.Angular_rates_z_rads = data.data(:, 14);
        ...
        log.Error_in_position_x_m = data.data(:, 27);
        log.Error_in_position_y_m = data.data(:, 28);
        log.Error_in_position_z_m = data.data(:, 29);
        
        log.Integral_Error_in_x_m = data.data(:, 30);
        log.Integral_Error_in_y_m = data.data(:, 31);
        log.Integral_Error_in_z_m = data.data(:, 32);
    
        log.Error_in_velocity_x_ms = data.data(:, 33);
        log.Error_in_velocity_y_ms = data.data(:, 34);
        log.Error_in_velocity_z_ms = data.data(:, 35);
    
        log.Error_in_Euler_phi_rad = data.data(:, 36);
        log.Error_in_Euler_theta_rad = data.data(:, 37);
        log.Error_in_Euler_psi_rad = data.data(:, 38);
        
        log.Integral_Error_in_phi_rads = data.data(:, 39);
        log.Integral_Error_in_theta_rads = data.data(:, 40);
        log.Integral_Error_in_psi_rads = data.data(:, 41);
    
        log.Error_in_Euler_phi_rate_rads = data.data(:, 42);
        log.Error_in_Euler_theta_rate_rads = data.data(:, 43);
        log.Error_in_Euler_psi_rate_rads = data.data(:, 44);
    
        log.Virutal_control_mu_in_I_x_N = data.data(:, 45);
        log.Virutal_control_mu_in_I_y_N = data.data(:, 46);
        log.Virutal_control_mu_in_I_z_N = data.data(:, 47);
    
        log.Virtual_control_in_body_x_N = data.data(:, 48);
        log.Virtual_control_in_body_y_N = data.data(:, 49);
        log.Virtual_control_in_body_z_N = data.data(:, 50);
    
        log.Control_input_u1_N = data.data(:, 51);
        log.Control_input_u2_Nm = data.data(:, 52);
        log.Control_input_u3_Nm = data.data(:, 53);
        log.Control_input_u4_Nm = data.data(:, 54);
    
        log.Thrust_Motor_1_N = data.data(:, 55);
        log.Thrust_Motor_2_N = data.data(:, 56);
        log.Thrust_Motor_3_N = data.data(:, 57);
        log.Thrust_Motor_4_N = data.data(:, 58);
    
        log.Thrust_Motor_1_Normalized_N = data.data(:, 59);
        log.Thrust_Motor_2_Normalized_N = data.data(:, 60);
        log.Thrust_Motor_3_Normalized_N = data.data(:, 61);
        log.Thrust_Motor_4_Normalized_N = data.data(:, 62);   

        log.Desired_phi_ddot_rads2 = data.data(:,63);
        log.Desired_theta_ddot_rads2 = data.data(:,64);
        log.Desired_psi_ddot_rads2 = data.data(:,65);
        ...
        file_name = ['PID_log_', flightRunNames{ii}, '.mat'];
