%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function is used to plot the Outer loop Theta hat gains along with 
...
function [] = plotOLTheta(log,der, title)

% Extract the numbers in the indices using regular expressions
fields = fieldnames(log.Theta_hat_tran);
...
for i = (1:length(m_ind))
    ...
    yline(der.outer_loop.projection_operator.up_bound.Theta, 'r--', 'LineWidth',2);
    yline(der.outer_loop.projection_operator.lw_bound.Theta, 'b-.', 'LineWidth',2);
    yline(-der.outer_loop.projection_operator.up_bound.Theta, 'r--', 'LineWidth',2);
    yline(-der.outer_loop.projection_operator.lw_bound.Theta, 'b-.', 'LineWidth',2);
    ...
    for j = (1:length(n_ind))
        ...
        gains = log.Theta_hat_tran.(fieldName);     % Extract the data
    ...
    proj_op_activated = log.proj_op_activated.outer_loop.Theta_hat;  % Boolean vector
