%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function is used to plot the Inner loop Theta hat gains along with 
...
function [] = plotILTheta(log,der, title)

% Extract the numbers in the indices using regular expressions
fields = fieldnames(log.Theta_hat_rot);
...
for i = (1:length(m_ind))
    ...
    yline(der.inner_loop.projection_operator.up_bound.Theta, 'r--', 'LineWidth',2);
    yline(der.inner_loop.projection_operator.lw_bound.Theta, 'b-.', 'LineWidth',2);
    yline(-der.inner_loop.projection_operator.up_bound.Theta, 'r--', 'LineWidth',2);
    yline(-der.inner_loop.projection_operator.lw_bound.Theta, 'b-.', 'LineWidth',2);
    ...
    for j = (1:length(n_ind))
        ...
        gains = log.Theta_hat_rot.(fieldName);     % Extract the data
    ...
    proj_op_activated = log.proj_op_activated.inner_loop.Theta_hat;  % Boolean vector
