%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function is used to plot the Outer loop K hat R gains along with the 
% projection operator
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [] = plotOLKr(log,der, title)

% Extract the numbers in the indices using regular expressions
fields = fieldnames(log.K_hat_r_tran);
...
for i = (1:length(m_ind))
    ...
    yline(der.outer_loop.projection_operator.up_bound.r, 'r--', 'LineWidth',2);
    yline(der.outer_loop.projection_operator.lw_bound.r, 'b-.', 'LineWidth',2);
    yline(-der.outer_loop.projection_operator.up_bound.r, 'r--', 'LineWidth',2);
    yline(-der.outer_loop.projection_operator.lw_bound.r, 'b-.', 'LineWidth',2);
    ...
    for j = (1:length(n_ind))
        ...
        gains = log.K_hat_r_tran.(fieldName);     % Extract the data
    ...
    proj_op_activated = log.proj_op_activated.outer_loop.K_hat_r;  % Boolean vector
